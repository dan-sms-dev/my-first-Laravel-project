    ...
    <main class="py-10">
        ...
        <section class="bg-white max-w-[600px] mx-auto p-8 pb-8 border-2 mt-4">
            ...
            <a href="{{ route('habits.create') }}"
                class="text-center p-2 border-2 bg-blue-500 text-white font-bold rounded-md w-auto max-w-[220px] block mx-auto mb-4">
                Cadastrar Hábito
            </a>
                    ...
                    @forelse ($habits as $item)
                        <li class="pl-4 text-center">
                            <div class="flex gap-2 items-center">
                                ...
                                <a class="bg-white-500 p-1 hover:opacity-60 cursor-pointer" href="{{ route('habits.edit', $item->id) }}">
                                  <x-icons.edit />
                                </a>
                                <form action="{{ route('habits.destroy', $item) }}" method="POST">
                                  @csrf
                                  @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white p-1 hover:opacity-60 cursor-pointer">
                                        <x-icons.trash />
                                    </button>
                                </form>
                        </li>
